...
get_link_git() {
  ...
  print_banner
  printf "${WHITE} 💻 Insira o link do GITHUB do MDOFLOW que deseja instalar:${GRAY_LIGHT}"
  printf "\n\n"
  read -p "> " link_git
}
...
inquiry_options() {
  ...
  print_banner
  printf "${WHITE} 💻 Bem vindo(a) ao Gerenciador MDOFlow, Selecione abaixo a proxima ação!${GRAY_LIGHT}"
  printf "\n\n"
  printf "   [0] Instalar MDOFlow BOT\n"
  printf "   [1] Atualizar MDOFlow\n"
  printf "   [2] Deletar MDOFlow BOT\n"
  printf "   [3] Bloquear MDOFlow\n"
  printf "   [4] Desbloquear MDOFlow\n"
  printf "   [5] Alter. dominio do MDOFlow\n"
  printf "\n"
  read -p "> " option
